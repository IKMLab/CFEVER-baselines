...
def get_doc_sent_file(sent_exp_dir: str, data_mode: str):
    pattern = os.path.join(sent_exp_dir, f"{data_mode}*.jsonl")

    # Find all files in the folder matching the pattern
    matching_files = glob.glob(pattern)
    assert len(matching_files) > 0, f"No sentence retrieval files found"
    assert (
        len(matching_files) == 1
    ), f"There should be only one certain file in the folder"
    print(f"Found {matching_files[0]} from the sent retrieval dir: {sent_exp_dir}")
